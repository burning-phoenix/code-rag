...
# -- Setup logging to stderr (MCP requirement: stdout is for protocol) --
logging.basicConfig(
    level=logging.INFO,
...
mcp = FastMCP(
    _config.name,
    instructions=_config.description,
)
...
def _format_result(r: dict, index: int, max_chars: int = 2000) -> str:
    ...
    # Format text as code block for code chunks
    if r.get("language"):
...
async def search_documents(
    ...
    from .indexer import get_client, search

    top_k = min(top_k, 20)

    try:
        query_vector = await embed_single(query, api_key=_config.openrouter_api_key)

        client = get_client(_config.qdrant_url)
        results = search(
            client,
            query_vector,
...
async def search_code(
    ...
    """
    from .embeddings import embed_single
    from .indexer import get_client, search, fetch_parent
    ...
    try:
        ...
        for i, r in enumerate(results, 1):
            ...
            if parent_id and parent_id not in seen_parents:
                ...
                parent = fetch_parent(client, _config.collection, parent_id)
                if parent:
...
async def lookup_index(concept: str) -> str:
    ...
    index_path = _config.pointer_index

    if not index_path.exists():
        return (
            "Pointer index not found. Run the ingest pipeline first:\n"
            "  code-rag ingest"
        )
...
def main():
    ...
    logger.info("Starting %s MCP server (STDIO transport)", _config.name)
