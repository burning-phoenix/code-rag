...
``scan`` extracts function/class/method names as pointer-index entries.
"""
...
def _get_parser(language: str) -> Any:
    ...
    if language not in _parser_cache:
        # Our LANGUAGE_CONFIG keys are a subset of get_parser's accepted Literal set.
        _parser_cache[language] = get_parser(language)  # type: ignore[arg-type]
...
def _node_lines(node: Any) -> int:
    """Number of lines spanned by a node (1-indexed inclusive)."""
    return int(node.end_point[0] - node.start_point[0] + 1)
    ...
    if node.type == "function_definition":
        ...
        if declarator:
            ...
            if declarator.type == "identifier":
                return str(declarator.text.decode("utf-8", errors="replace"))
...
def _get_class_methods(node: Any, lang_config: dict[str, Any]) -> list[Any]:
...
def _flush_preamble(nodes: list, source: bytes, file_name: str, language: str) -> list[Chunk]:
    ...
    text = (
        source[nodes[0].start_byte : nodes[-1].end_byte].decode("utf-8", errors="replace").strip()
    )
...
def _make_function_chunk(node: Any, source: bytes, file_name: str, language: str) -> Chunk:
...
def _is_mergeable(chunk: Chunk) -> bool:
    """A chunk may be merged only if it's a standalone, role-free sibling.

    Class parents and their methods carry the parent→child structure (linked via
    ``chunk_id``/``parent_id``); folding either into a neighbour would corrupt
    that structure and erase the method as a retrievable unit. Only preambles and
    top-level functions (no ``parent_id``, not a class/method) are foldable.
    """
    return chunk.parent_id is None and chunk.symbol_type not in ("class", "method")


def _merge_tiny_chunks(chunks: list[Chunk], min_lines: int) -> list[Chunk]:
    """Merge tiny standalone chunks into the previous standalone sibling."""
    ...
    for chunk in chunks:
        ...
        if (
            ...
            and result[-1].language == chunk.language
            and _is_mergeable(result[-1])
            and _is_mergeable(chunk)
        ):
...
class CodeChunker(BaseChunker):
    """AST chunker for all tree-sitter supported languages."""
    ...
    def chunk(self, file_path: Path, config: ProjectConfig) -> list[Chunk]:
        ...
        lang_config = LANGUAGE_CONFIG.get(language) if language else None

        if not language or not lang_config:
            logger.warning("No language config for %s, skipping", file_name)
            return []
        ...
        for child in root.children:
            if child.type in function_types:
                ...
                chunks.extend(
                    _chunk_class_node(
                        child, source, file_name, language, lang_config, max_chunk_lines
                    )
                )
    ...
    def scan(self, file_path: Path, config: ProjectConfig) -> list[IndexEntry]:
        """Scan a code file for function/class/method definitions."""
        file_path = Path(file_path)
        language = EXTENSION_TO_LANGUAGE.get(file_path.suffix.lower())
        lang_config = LANGUAGE_CONFIG.get(language) if language else None
        if not language or not lang_config:
